...
export default function AddOrder() {
    ...
    const fetchT = async () => {
      ...
      if (id) {
        ...
        if (order) {
          ...
          if (INDIAN_CLOTH_TYPES.includes(order.cloth_type)) {
            setClothType(order.cloth_type);
          } else {
            setClothType('Other');
            setCustomClothType(order.cloth_type);
          }
          
          setFabricImage(order.image_url);
          setMeasurementImage(order.measurement_image_url);
          setInstructions(order.instructions_text || '');
          setTailorId(order.tailor_id);
        }
      } else if (data && data.length > 0) {
        setTailorId(data[0].id || data[0].name); 
      }
    };
    fetchT();
  }, [id]);

  const handleImageCapture = (e, setter) => {
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    const finalClothType = clothType === 'Other' ? customClothType : clothType;

    if (!customerName || !finalClothType || !customerPhone) {
    ...
    const payload = {
      ...
      cloth_type: finalClothType,
      instructions_text: instructions,
      tailor_id: tailorId,
      image_url: fabricImage,
      measurement_image_url: measurementImage
    };

    if (id) {
      await addOfflineAction('UPDATE_ORDER', { order_id: id, ...payload });
    } else {
      await addOfflineAction('CREATE_ORDER', payload);
    }
  ...
  return (
    <div className="bg-white dark:bg-[#1a1a1a] p-[12px] sm:p-4 md:p-10 rounded-xl md:rounded-[2.5rem] shadow-sm md:shadow-2xl border border-gray-100 dark:border-gray-800 max-w-2xl mx-auto space-y-4 md:space-y-8 mb-4 md:mb-12 transition-colors">
      <div className="flex items-center gap-2 md:gap-4 border-b border-gray-100 dark:border-gray-800 pb-3 md:pb-6">
        ...
        <h2 className="text-xl md:text-3xl font-black text-gray-900 dark:text-gray-100 tracking-tight uppercase">{id ? t('edit_order') : t('new_order')}</h2>
      </div>
      ...
      <form onSubmit={handleSave} className="space-y-4 md:space-y-8">
        ...
        <div>
          <label className="block text-gray-800 dark:text-gray-300 font-bold text-sm md:text-lg mb-2 md:mb-4 border-t border-gray-100 dark:border-gray-800 md:border-t-2 pt-3 md:pt-6">{t('select_cloth')}</label>
          <div className="flex flex-wrap gap-2 md:gap-3">
            {INDIAN_CLOTH_TYPES.map(type => (
              <button 
                key={type}
                type="button"
                ...
                className={`py-[6px] px-3 md:py-3 md:px-5 rounded-lg md:rounded-xl text-xs md:text-base font-bold transition-all border flex-grow sm:flex-grow-0 text-center ${
                  clothType === type ? 'bg-indigo-600 dark:bg-indigo-500 text-white border-indigo-600 dark:border-indigo-500 shadow-sm md:shadow-md' : 'bg-white dark:bg-[#252525] border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:border-indigo-300 shadow-sm'
                }`}
              >
                {type}
              </button>
        ...
        <div className="grid grid-cols-2 gap-3 md:gap-6">
          <div className="flex flex-col gap-1.5 md:gap-3">
             <label className="block text-gray-800 dark:text-gray-300 font-bold text-sm md:text-lg w-max shadow-none">{t('fabric_photo')}</label>
             <label className="flex flex-col items-center justify-center p-3 md:p-6 border md:border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-xl md:rounded-3xl bg-gray-50 dark:bg-[#252525] cursor-pointer hover:border-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-colors h-24 md:h-40 relative overflow-hidden group">
                {fabricImage ? (
                   <img loading="lazy" src={getImageUrl(fabricImage)} className="absolute inset-0 w-full h-full object-cover group-hover:opacity-50 transition-opacity" />
                ) : (
                   <><Camera size={24} className="text-gray-400 dark:text-gray-500 mb-1 md:mb-3 md:w-10 md:h-10" /><span className="text-gray-500 dark:text-gray-400 font-bold text-[10px] md:text-sm text-center leading-tight">{t('tap_snap')}</span></>
                )}
                <input type="file" accept="image/*" capture="environment" className="hidden" onChange={(e) => handleImageCapture(e, setFabricImage)} />
             </label>
          </div>
          <div className="flex flex-col gap-1.5 md:gap-3">
             <label className="block text-gray-800 dark:text-gray-300 font-bold text-sm md:text-lg w-max shadow-none">{t('measurement')}</label>
             <label className="flex flex-col items-center justify-center p-3 md:p-6 border md:border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-xl md:rounded-3xl bg-gray-50 dark:bg-[#252525] cursor-pointer hover:border-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-colors h-24 md:h-40 relative overflow-hidden group">
                {measurementImage ? (
                   <img loading="lazy" src={getImageUrl(measurementImage)} className="absolute inset-0 w-full h-full object-cover group-hover:opacity-50 transition-opacity" />
                ) : (
                   <><ImageIcon size={24} className="text-gray-400 dark:text-gray-500 mb-1 md:mb-3 md:w-10 md:h-10" /><span className="text-gray-500 dark:text-gray-400 font-bold text-[10px] md:text-sm text-center leading-tight">{t('scan_paper')}</span></>
                )}
                <input type="file" accept="image/*" capture="environment" className="hidden" onChange={(e) => handleImageCapture(e, setMeasurementImage)} />
             </label>
          </div>
        </div>
        ...
        <div>
           <label className="block text-gray-800 dark:text-gray-300 font-bold text-sm md:text-lg mb-2 md:mb-4 border-t pt-3 md:border-t-2 md:pt-6 border-gray-100 dark:border-gray-800">{t('assign_tailor')}</label>
           {tailors.length === 0 ? (
             ...
             <div className="relative">
               ...
               >
                 {tailors.map((t, idx) => (
                   <option key={t.id || idx} value={t.id || t.name}>{t.name}</option>
                 ))}
               </select>
               <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-4 md:px-6 text-gray-700 dark:text-gray-400">
                 <svg className="fill-current h-4 w-4 md:h-6 md:w-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"/></svg>
               </div>
             </div>
           )}
        </div>
        ...
        <button 
          type="submit" 
          className="w-full bg-green-500 hover:bg-green-600 dark:bg-green-600 dark:hover:bg-green-500 text-white font-bold md:font-black py-3 md:py-5 mt-4 md:mt-8 rounded-xl md:rounded-[2rem] text-base md:text-2xl flex items-center justify-center gap-2 md:gap-3 active:scale-95 transition-all outline-none focus:ring-4 md:focus:ring-8 focus:ring-green-200 dark:focus:ring-green-900 border border-green-400 md:border-4 dark:border-none shadow-sm md:shadow-lg"
        >
          <Save size={20} className="md:w-8 md:h-8" /> {id ? t('update_order') : t('save_order')}
        </button>
      </form>
    </div>
  );
